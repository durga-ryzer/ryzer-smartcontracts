...
contract RyzerTokenManagement is Initializable, UUPSUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdrawERC20(address token, uint256 amount) external onlyRole(CUSTODIAN_ROLE) notEmergencyStopped nonReentrant {
        if (token == address(0) || amount == 0) revert InvalidToken();
        IERC20(token).safeTransfer(msg.sender, amount);
        emit TokenWithdrawn(msg.sender, token, amount, false, 0);
    }

    function withdrawERC721(address token, uint256 tokenId) external onlyRole(CUSTODIAN_ROLE) notEmergencyStopped nonReentrant {
        if (token == address(0)) revert InvalidToken();
