...
contract RyzerCrossChain is RyzerWalletCore {
    ...
    mapping(address => mapping(address => mapping(uint256 => uint256))) internal userERC1155Balances;
    ...
    event CrossChainFeeCached(uint256 indexed chainId, address indexed token, uint256 fee);
    ...
    event CrossChainAdapterPriorityUpdated(address indexed adapter, uint32 priority);
    ...
    event AdapterPerformanceUpdated(address indexed adapter, bool success, uint256 latency);
    ...
    function initializeCrossChain(
        ...
        RyzerWalletCore.initialize(RyzerWalletCore.InitParams({
        ...
        if (ccipRouterAddress == address(0) || params.oracleAggregators.length == 0) {
        ...
        if (params.crossChainAdapters.length != params.adapterPriorities.length) {
        ...
        for (uint256 i = 0; i < params.crossChainAdapters.length; i++) {
            ...
            crossChainAdapters[adapterAddress] = AdapterInfo(adapterAddress, priority);
    ...
    /// @param priority Adapter priority
    function addCrossChainAdapter(address adapterAddress, uint32 priority) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant notEmergencyStopped {
        if (adapterAddress == address(0) || crossChainAdapters[adapterAddress].adapter != address(0)) {
            revert InvalidAdapter();
        }
    ...
    function removeCrossChainAdapter(address adapterAddress) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant notEmergencyStopped {
    ...
    /// @param adapterAddress Adapter address
    /// @param priority New priority
    function updateAdapterPriority(address adapterAddress, uint32 priority) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant notEmergencyStopped {
        if (crossChainAdapters[adapterAddress].adapter == address(0)) {
            revert AdapterNotFound();
        }
        if (priority > MAX_PRIORITY) {
    ...
    /// @param success Whether the transfer succeeded
    /// @param latency Transfer latency in seconds
    function updateAdapterPerformance(address adapterAddress, bool success, uint256 latency) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant notEmergencyStopped {
        if (crossChainAdapters[adapterAddress].adapter == address(0)) {
            revert AdapterNotFound();
        }
        ...
        uint256 successRate = (perf.successCount * 100) / (perf.successCount + perf.failureCount);
        uint256 avgLatency = perf.transferCount > 0 ? perf.totalLatency / perf.transferCount : 0;
        uint32 newPriority = crossChainAdapters[adapterAddress].priority;
        if (successRate >= 95 && avgLatency < 30) {
            newPriority = newPriority < MAX_PRIORITY - 10 ? newPriority + 10 : MAX_PRIORITY;
    ...
    function initiateCrossChainTransfer(TransferParams memory transferParams) external onlyRole(CUSTODIAN_ROLE) nonReentrant whenNotPaused notBlacklisted(transferParams.user) notBlacklisted(transferParams.recipient) rateLimited(transferParams.user) notEmergencyStopped {
        ...
        (uint256 crossChainFee, uint256 walletFee, uint256 amountAfterWalletFee) = _calculateFees(transferParams.token, transferParams.amount, transferParams.targetChainId);
        ...
        _handleTokenApprovals(transferParams, metadata, walletFee, amountAfterWalletFee, selectedAdapter);
        (success, txHash) = _executeCcipTransfer(transferParams, amountAfterWalletFee, selectedAdapter);
    ...
    function _validateRecipientCompliance(address recipient, TokenMetadata memory metadata) internal view {
        if (metadata.complianceLevel >= 1) {
            if (!hasRole(CUSTODIAN_ROLE, recipient) && !hasRole(BROKER_ROLE, recipient)) {
    ...
    /// @param transferParams Transfer parameters
    /// @param metadata Token metadata
    function _revertTokenBalances(TransferParams memory transferParams, TokenMetadata memory metadata) internal {
        if (metadata.tokenStandard == type(IERC20).interfaceId || metadata.tokenStandard == type(IERC3643).interfaceId) {
            userTokenBalances[transferParams.user][transferParams.token] += transferParams.amount;
        } else if (metadata.tokenStandard == type(IERC721).interfaceId) {
            tokenToIdToUser[transferParams.token][transferParams.tokenIds[0]] = transferParams.user;
        } else {
            for (uint256 i = 0; i < transferParams.tokenIds.length; i++) {
                userERC1155Balances[transferParams.user][transferParams.token][transferParams.tokenIds[i]] += transferParams.amounts[i];
            }
        }
    }
    ...
    function _executeCcipTransfer(
        ...
        Client.EVM2AnyMessage memory message = _constructCcipMessage(transferParams, amountAfterWalletFee);
        try IRouterClient(selectedAdapter).ccipSend(uint64(transferParams.targetChainId), message) returns (bytes32 _txHash) {
            txHash = _txHash;
    ...
    function _constructCcipMessage(
        ...
        return Client.EVM2AnyMessage({
            ...
            data: abi.encode(transferParams.token, amountAfterWalletFee, transferParams.tokenIds, transferParams.amounts, transferParams.params),
    ...
    function _calculateFees(address token, uint256 amount, uint256 targetChainId) internal returns (uint256 crossChainFee, uint256 walletFee, uint256 amountAfterWalletFee) {
        crossChainFee = cachedCrossChainFees[targetChainId][token];
        if (crossChainFee == 0 || block.timestamp > feeCacheExpiries[targetChainId][token]) {
            crossChainFee = estimateCrossChainFee(token, amount, targetChainId);
            cachedCrossChainFees[targetChainId][token] = crossChainFee;
            feeCacheExpiries[targetChainId][token] = uint64(block.timestamp + FEE_CACHE_DURATION);
    ...
    /// @param transferParams Transfer parameters
    /// @param metadata Token metadata
    function _checkTokenBalances(TransferParams memory transferParams, TokenMetadata memory metadata) internal {
        if (metadata.tokenStandard == type(IERC20).interfaceId || metadata.tokenStandard == type(IERC3643).interfaceId) {
            if (userTokenBalances[transferParams.user][transferParams.token] < transferParams.amount) {
            ...
            if (tokenToIdToUser[transferParams.token][transferParams.tokenIds[0]] != transferParams.user) {
                revert NotTokenOwner();
            }
            tokenToIdToUser[transferParams.token][transferParams.tokenIds[0]] = address(0);
        } else {
            for (uint256 i = 0; i < transferParams.tokenIds.length; i++) {
                if (userERC1155Balances[transferParams.user][transferParams.token][transferParams.tokenIds[i]] < transferParams.amounts[i]) {
                    revert InsufficientBalance();
                }
                userERC1155Balances[transferParams.user][transferParams.token][transferParams.tokenIds[i]] -= transferParams.amounts[i];
            }
    ...
    function _selectCrossChainAdapter(
        ...
        AdapterSelection memory selection = AdapterSelection(address(0), type(uint256).max, 0);

        if (IRouterClient(ccipRouter).isChainSupported(uint64(transferParams.targetChainId))) {
            uint256 fee = IRouterClient(ccipRouter).getFee(
                uint64(transferParams.targetChainId),
                Client.EVM2AnyMessage({
                    receiver: abi.encode(address(0)),
                    data: abi.encode(transferParams.token, transferParams.amount, new uint256[](0), new uint256[](0), transferParams.params),
                    tokenAmounts: new Client.EVMTokenAmount[](0),
                    extraArgs: "",
                    feeToken: address(0)
                })
        ...
        for (uint256 i = 0; i < adapterList.length; i++) {
            ...
            uint256 successRate = perf.transferCount > 0 ? (perf.successCount * 100) / perf.transferCount : 100;
            if (successRate < MIN_ADAPTER_SUCCESS_RATE) {
                revert UnhealthyAdapter();
            }
            if (info.adapter != address(0) && ICrossChainAdapter(adapterAddress).isChainSupported(uint64(transferParams.targetChainId))) {
                ...
                fee += (fee * tx.gasprice) / 1e18;
                if (selection.adapter == address(0) || (info.priority > selection.priority) || (info.priority == selection.priority && fee < selection.fee)) {
                    selection = AdapterSelection(adapterAddress, fee, info.priority);
                }
            }
        }
    ...
    function estimateCrossChainFee(address token, uint256 amount, uint256 targetChainId) internal returns (uint256) {
        int256 price;
        uint256 latestUpdate;
        for (uint256 i = 0; i < oracleAggregators.length; i++) {
            try IOracleAggregator(oracleAggregators[i]).latestRoundData() returns (
                uint80,
                int256 _price,
                uint256,
                uint256 updatedAt,
                ...
                if (_price > 0 && block.timestamp <= updatedAt + ORACLE_STALENESS_THRESHOLD) {
                    price = _price;
        ...
        (, uint256 baseFee) = _selectCrossChainAdapter(TransferParams({
            user: address(0),
            token: token,
            amount: amount,
            recipient: address(0),
    ...
    /// @param newFeeRecipient New fee recipient address
    function setFeeRecipient(address newFeeRecipient) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant notEmergencyStopped {
    ...
    /// @param newFeePercentage New fee percentage (in basis points)
    function setFeePercentage(uint64 newFeePercentage) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant notEmergencyStopped {
    ...
    function getAdapterInfo(address adapterAddress) external view returns (AdapterInfo memory adapterInfo, AdapterPerformance memory adapterPerf) {
        return (crossChainAdapters[adapterAddress], adapterPerformance[adapterAddress]);
    }
}