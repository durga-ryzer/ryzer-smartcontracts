    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(uint16 _chainId) external initializer {
        if (_chainId == 0 || _chainId != uint16(block.chainid)) revert InvalidChainId(_chainId);
    ...
    function _validateCompanyInput(address owner, string memory name, string memory jurisdiction) private pure {
        ...
        if (bytes(name).length == 0 || bytes(name).length > MAX_STRING_LENGTH) revert InvalidParameter("name");
    ...
    function _validateProjectInput(ProjectParams calldata params) private pure {
        ...
        if (bytes(params.metadataCID).length == 0) revert InvalidParameter("metadataCID");
        if (bytes(params.legalMetadataCID).length == 0) revert InvalidParameter("legalMetadataCID");
        if (params.projectAddress == address(0)) revert InvalidAddress(params.projectAddress);
    ...
    function updateLegalMetadataCID(uint256 companyId, uint256 projectId, string calldata newCID)
        ...
        whenNotPaused
    {
        if (bytes(newCID).length == 0) revert InvalidParameter("legalMetadataCID");
