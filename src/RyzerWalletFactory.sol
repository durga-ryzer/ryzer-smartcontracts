...
contract RyzerWalletFactory is AccessControl, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        require(
            _walletImplementation != address(0) && _entryPoint != address(0),
            "Invalid addresses"
        );
        require(
            _paymaster != address(0) && _timelock != address(0),
            "Invalid addresses"
        );
        require(
            _walletImplementation.code.length > 0,
    ...
    function createWallet(
        WalletParams calldata params
    ) external whenNotPaused nonReentrant returns (address payable wallet) {
    ...
    function _validateParams(WalletParams calldata p) private view {
        ...
        require(
            p.custodians.length <= 50 && p.brokers.length <= 50,
            "Too many roles"
        );
    ...
    function _deployWallet(
        WalletParams calldata p
    ) private returns (address payable wallet) {
        wallet = payable(Clones.clone(walletImplementation));
        isWallet[wallet] = true;

        try
            RyzerWalletCore(wallet).initialize(
